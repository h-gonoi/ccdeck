...
### サイズの持ち主の通知に、測り直しで応えてはいけない

PC の窓が二つ以上あると、`sizeOwner` の通知 → 測り直して `resize` → 持ち主交代 → また通知 … の往復が
**秒に数十回**起き、PTY の桁数が 67×60 ↔ 97×31 のように揺れ続ける（実際に起きた。PC が点滅し、
描画途中の画面で状態判定が揺れ、スマホの住人の並びまで入れ替わり続けた）。

`web/main.js` の `onSizeOwner` は、持ち主が自分で桁数が違うときだけ一度だけ合わせ直し、
奪われた側は手前に居て桁数が違うときだけ 10 秒に一度取り返す。**通知のたびに `pool.setOwned()` を呼ぶ形に戻さないこと。**
サーバー側も `sizeStorm()` で持ち主の切り替えが 3 秒に 6 回を超えたら受け付けを止め、
同じ桁数の `resize` は PTY に触らない。点滅を見たら、まず `sessions` の `cols×rows` が揺れていないかを見る
（WebSocket で `sessions` を購読して数えれば 20 秒で判る）。
...
上へ遡り、番号付きの行を選択肢として拾う。番号が無い問い（フォルダの信頼）は `❯` の桁に揃う行を集める。
罫線（`────` や `▔▔▔`）は箱の上端なので、質問文はそこで切る。**「末尾 N 行」に戻さないこと。**

選ぶのは番号があればその数字、無ければ `❯` の位置から矢印で動かして Enter（`keysFor`）。
1・2・3 の固定ボタンに戻すと、6 択の問いに答えられなくなる。

### モデルは CLI の記録から読み、切り替えは CLI に任せる

`server/transcripts.js` の `modelFor()` が会話記録の最後の `"model"` を拾う（`<synthetic>` は飛ばす。
これを拾うと「モデル: <synthetic>」と出る）。記録のサイズと更新時刻が同じなら読み直さない。
表示は `prettyModel()` で `claude-fable-5-1` → `Fable 5.1` に均す。

切り替えは**独自の一覧を持たず `/model` を送る**。CLI が出す一覧がその場の正しい選択肢で、
それは上の `parsePrompt` がそのまま選択肢として拾える。自前のモデル名一覧を持つと必ず古くなる。
...
### スマホの文字は WebView に描かせる

iOS の `Text` は `selectable` を付けても**全文しか選べない**。一部だけコピーしたくて
編集不可の `TextInput` に替えたら、今度は**中身より高い枠を取って大きな空白が空いた**（両方とも実際に起きた）。
会話と画面は `mobile/scene/chat.html` を WebView に載せて描いている。選択とコピーは WKWebView に任せ、
Markdown は `mobile/src/markdown.ts` が HTML に直してから渡す（生の HTML は通さない。
返答に `<script>` が混ざっても文字として出す）。入力欄・キーバー・選択肢はネイティブのまま。
...
- **執事は自分の席で動く。** `<名前>(執事)` のセッションを立てて使い、主人が開いているセッションには入らない
  （打ちかけの入力を奪い、会話の筋も混ざる）。席は場所ごとに一つで、巡をまたいで使い回す
- **上限に当たったら止める。** 使い切りの合図（`usage limit` など）を見つけたら巡を `paused` にし、
  渡していない手順は次に回す。ここを素通しすると、空振りしたまま一巡したことにして
  **次の見立てへ進み、提案の画面に戻ってしまう**（実際に戻った）
...
- 会話の文字を一部だけ選んでコピーでき、余分な空白が空いていないこと（WebView で描いている）
- 返答の Markdown（見出し・箇条書き・コード・表）が整って出ること
- 承認待ちの選択肢が**全部**ボタンとして並び、押すとその選択肢が選ばれること
- モデル名が出ること。押すと `/model` の一覧が選択肢として並ぶこと
- 画像を添付すると `~/.ccdeck/attachments/` に落ち、パスが文章に入ること
...
- 手順が `<名前>(執事)` のセッションで動き、主人のセッションに入力されないこと
- 上限に当たったら `paused` で止まり、次の見立てを勝手に始めないこと
